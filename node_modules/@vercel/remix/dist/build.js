...
const build = async ({ entrypoint, files, workPath, repoRootPath, config, meta = {}, }) => {
    ...
    const remixDevPackageJsonPath = _require.resolve('@remix-run/dev/package.json', { paths: [entrypointFsDirname] });
    const remixVersion = JSON.parse(await fs_1.promises.readFile(remixDevPackageJsonPath, 'utf8')).version;
...
module.exports = config;`;
    ...
    const [staticFiles, nodeFunction, edgeFunction] = await Promise.all([
        ...
        createRenderNodeFunction(entrypointFsDirname, repoRootPath, (0, path_1.join)(entrypointFsDirname, serverBuildPath), remixConfig.serverEntryPoint, nodeVersion, remixVersion),
        hasEdgeRoute
            ? createRenderEdgeFunction(entrypointFsDirname, repoRootPath, (0, path_1.join)(entrypointFsDirname, serverBuildPath), remixConfig.serverEntryPoint, remixVersion)
            : undefined,
    ]);
...
async function createRenderNodeFunction(entrypointDir, rootDir, serverBuildPath, serverEntryPoint, nodeVersion, remixVersion) {
    const files = {};
...
async function createRenderEdgeFunction(entrypointDir, rootDir, serverBuildPath, serverEntryPoint, remixVersion) {
    const files = {};
    let handler = (0, path_1.relative)(rootDir, serverBuildPath);
    let handlerPath = (0, path_1.join)(rootDir, handler);
    if (!serverEntryPoint) {
            ...
            if ((0, path_1.basename)(fsPath) === 'package.json') {
                ...
                if (pkgJson.name === '@remix-run/vercel') {
                    ...
                    if (!remixRunVercelPkgJson) {
                        ...
                        const vercelEdgeEntrypointPath = (0, path_1.join)(__dirname, '../vercel-edge-entrypoint.js');
